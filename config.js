	...
	$.ajax({
        ...
        data.forEach(function(booking) {
        	if(booking.roomType === type && start <= booking.checkIn.substring(0,2) && booking.checkIn.substring(0,2) <= end) {   // Check for the room Type and Date of booking
        		const days = booking.checkOut.substring(0,2) - booking.checkIn.substring(0,2);
        		if(flag === 0) {
        			let header;
	        		if(start === 1) {
		        		header = `
        		...
        		const daysPadding = days*100;
        		const styleLeft = checkInMargin( (booking.checkIn.substring(0,2)) % 7 );
        		const id = document.getElementById(booking.roomNumber);
        		const append = `<p class="booked" style="left:${styleLeft}px;width:${daysPadding}px;"> Booked </p>`
        		$(id).append(append);
        	}
        });
    });
